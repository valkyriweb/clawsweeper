...
Read when changing ClawSweeper throughput, model-worker fan-out, commit review paging,
...
the number we normally tune when model-provider or GitHub rate limits get tight. Most
...
- `workers.max` is the global model-worker capacity budget.
- Priority lanes are repair, issue implementation, docs maintenance, and exact-item review.
...
| --- | ---: | --- |
| `workers.max` | 8 | Maximum global model-worker budget used to derive lane limits. |
...
commit review can use 1 commit per page, repair/issue lanes can dispatch 3 live
workers, and docs maintenance can dispatch 2 live workers.
...
| `repair_live_runs.docs_maintenance_default` | 2 | Live repair run cap for PR-scoped docs maintenance. |
...
- docs maintenance: 25% of `workers.max`
...
the first run before its shard jobs exist and over-allocates the shared model-worker
...
- `CLAWSWEEPER_MAX_LIVE_WORKERS` can also override docs-maintenance dispatches;
  otherwise `job_intent: docs_maintenance` uses
  `repair_live_runs.docs_maintenance_default`.
