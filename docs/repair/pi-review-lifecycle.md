...
Completion requires an immutable trusted-bot comment containing exactly one `pi-pr-review-lifecycle` JSON marker matching that identity and verdict (`pass`, `fail`, `incomplete`). The technical check must be `pi-pr-review/verdict`, from GitHub Actions, on the candidate SHA, with external ID `pi-pr-review:<run>:<attempt>`. The legacy details URL `https://github.com/<repo>/actions/runs/<run>/attempts/<attempt>` remains supported. GitHub's normalized native details/HTML URL `https://github.com/<repo>/runs/<check-id>` is accepted only when both URLs match the validated check ID and that ID equals `check_id` in the authenticated, run-owned lifecycle artifact. Both IDs must be positive safe integers. GitHub can assign custom checks to another workflow's suite, so check-suite equality is not assumed. These URL shapes do not relax the other artifact, comment, SHA or attempt guards. Pass requires success; fail and incomplete require failure.
...
Worker source checkouts explicitly select the full tree with non-cone `/*`. The same persistent runner may previously have run a sparse router checkout. In that state, checkout's default `sparse-checkout disable` followed by removing `extensions.worktreeConfig` can expose the old repository-local sparse setting again; the next checkout then omits worker scripts. The explicit full-tree pattern avoids that sequence without changing the intended worker source scope. `test/repair/worker-checkout.test.ts` reproduces the transition in isolated Git repositories for both worker jobs. Do not repair the runner's active checkout manually or reset repair budgets after a bootstrap failure.

The router loads the same `CLAWSWEEPER_MODELS` variable as the executor and runs `workflow -- models list` before token creation, job adoption or dispatch. This validates the whole configured registry, including unrelated action entries that executor startup also parses. A registry mismatch therefore fails before spending an automatic repair receipt. It is a local catalog/configuration preflight, not proof of live provider availability. Catalog updates must check the actual transport's live models; Pi selectors and Codex model strings are not interchangeable. The executor honors the existing `CLAWSWEEPER_CODEX_AUTH_MODE` variable (subscription remains the unset default), matching commit-review; ClawRouter mode exposes the existing device-key secret only to Codex setup and execution, without changing provider policy or credentials. Plan-only worker runs still do not prove executor/model readiness.

Deterministic repair artifacts retain `pnpm check:changed` for `openclaw/openclaw` and the managed `valkyriweb/openclaw` fork. The other registered full-automation targets were checked for that script before selecting a different baseline. Other targets receive `git diff --check` as a baseline; the edit pass must discover and run their own relevant tests. This avoids inventing a package script for repositories with no root package manifest. It does not replace native review of the repaired head.
