...
test("workflow utilities expose review capacity telemetry from plans", () => {
  assert.deepEqual(
    planOutputFields(
      {
        ...
        reviewProvider: "pi",
        reviewPolicy: { model: "clawrouter/claude-opus-5-200k" },
      },
    ...
    ),
    {
      ...
      review_model: "clawrouter/claude-opus-5-200k",
      review_provider: "pi",
      due_backlog: "17",
...
test("codexReasoningEffortForAction resolves override, then env, then default", () => {
  ...
  try {
    ...
    // Default: commit-review is medium, sweep-review is none.
    assert.equal(codexReasoningEffortForAction("commit-review"), "medium");
...
test("actionModel resolves registry override, then fallback, then default", () => {
  ...
  try {
    ...
    assert.equal(actionModel("sweep-review", ""), "clawrouter/claude-opus-5-200k");
...
test("actionProvider resolves registry override, then validated fallback, then default", () => {
  ...
  try {
    ...
    // Registry override wins over the fallback.
    process.env.CLAWSWEEPER_MODELS = JSON.stringify({
      "commit-review": { provider: "pi", model: "clawrouter/claude-opus-5-200k" },
    });
